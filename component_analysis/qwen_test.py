...
model = Qwen2VLForConditionalGeneration.from_pretrained(
    model_name, torch_dtype="auto"  # Map all components to the same device
).cuda(7)
...
# randomly shuffle the object_test dataset and take the first 500 samples
random.seed(42)
random.shuffle(object_test)
...
def direction_train_func():
    ...
    linear_model = LinearProbe(input_dim=1280).to(device)
...
def object_train_func():
    ...
    linear_model = LinearProbe(input_dim=1280, num_classes=90).to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(linear_model.parameters(), lr=0.001)
