...
# randomly select 101 samples
object_test = object_test[:500]
...
def direction_train_func():
    ...
    for input_pair in tqdm.tqdm(direction_train):
        ...
        # get the CLS 
        # print(outputs.keys())
        # print("outputs", outputs["last_hidden_state"].shape) # [1, 1, 4, 1025, 7680]
        # get the 1 * 7068 cLS TOKEn
        CLS_token = outputs["last_hidden_state"][:, 0, 0,0, :].cpu().detach().float()
    ...
    linear_model = LinearProbe(input_dim=7680).to(device)
...
def object_detect_func():
    ...
    linear_model = LinearProbe(input_dim=7680, num_classes=90).to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(linear_model.parameters(), lr=0.001)
    # Train the model
    num_epochs = 50
