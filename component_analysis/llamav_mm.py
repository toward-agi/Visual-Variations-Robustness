from transformers import MllamaForConditionalGeneration, AutoProcessor, MllamaVisionModel
import torch
...
vit = MllamaVisionModel.from_pretrained(model_id).to("cuda:6").eval()
model = MllamaForConditionalGeneration.from_pretrained(
    model_id,
    torch_dtype=torch.bfloat16,
)
mllama_mm_proj = model.multi_modal_projector
...
obj_train = obj_train.select(range(800))
...
object_test = random.sample(object_test, 500)
...
def direction_train_func():
    ...
    for input_pair in tqdm.tqdm(direction_train):
        # prompt = "<|image|><|begin_of_text|>What is inside the image?"

        inputs = processor(images=input_pair["image"], return_tensors="pt").to("cuda:6")
        inputs = {k: v.to(torch.bfloat16) if torch.is_floating_point(v) else v for k, v in inputs.items()}
        cross_attention_states = vit(**inputs).last_hidden_state
        # print(cross_attention_states[0].shape)
        # print(mllama_mm_proj(cross_attention_states[0].to(torch.bfloat16)).shape) # [1, 4, 1025, 4096]
        outputs = mllama_mm_proj(cross_attention_states[0].to(torch.bfloat16)).squeeze()
        outputs = outputs[:,0,:].squeeze().mean(dim=0)
        # print("out shape",outputs.shape)
        CLS_token = outputs.cpu().detach().float()
        
        CLS_tokens.append(CLS_token.unsqueeze(dim=0).float()) 

    # Convert CLS tokens and labels to tensors
    CLS_tokens = torch.tensor(torch.cat(CLS_tokens, axis=0))
    print("CLS_tokens", CLS_tokens.shape)
    train_labels = torch.tensor([data["rotation_angle"] // 45 for data in direction_train])
    linear_model = LinearProbe(input_dim=4096).to(device)
...
def object_detect_func():
    ...
    linear_model = LinearProbe(input_dim=4096, num_classes=90).to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(linear_model.parameters(), lr=0.001)
    # Train the model
    num_epochs = 100
